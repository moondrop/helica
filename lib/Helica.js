...
class Helica {
    ...
    /**
     * Read files from an entr point recursively
     * @async
     * @private
    ...
    async serveStatic(directory) {
        ...
        for (const file of files) {
            ...
            this.staticRoutes.push({ route, mimeType, content });
        }
        ...
        if (this.staticRoutes.length > 0) {
            for (const staticRoute of this.staticRoutes) {
                ...
                this.server.get(staticRoute.route, (response) => {
