...
class Helica {
    ...
    _serializeRequest(request, route) {
        ...
        return { headers, method: HTTPMethod, parameters, query, url, route };
    }
     ...
     * @returns {Array<String>} An array with all files
     */
    _walkDirectory(directory) {
        let results = [];
        const list = fs.readdirSync(directory);

        for (const potentialFile of list) {
            const file = path.join(directory, potentialFile);
            const fileStats = fs.statSync(file);

            if (fileStats && fileStats.isDirectory()) {
                results = results.concat(this._walkDirectory(file));
            } else {
                results.push(file);
            }
        }
     ...
     */
    serveStatic(directory, basePath = '/') {
        if (this.running) {
        ...
        const files = this._walkDirectory(directory);
     ...
     * @param {Function|Promise<Function>} middleware A promise returning a function or a synchronous function
     */
    addMiddleware(middleware) {
        if (typeof middleware === 'object') {
            this.internalMiddlewares.push(middleware.name);
            this.globalMiddlewares.push(middleware.executor);
        } else {
    ...
    addResource(route, resourceHandler) {
        if (this.internalMiddlewares.includes('__cors')) {
            if (!(typeof resourceHandler.prototype.options === 'function')) {
                resourceHandler.prototype.options = (response) => {
                    response.end();
                };
            }
        }
                ...
                if (requestMethod.toUpperCase() === 'DELETE') {
                    this.server.del(route, async (response, request) => {
                        ...
                        try {
                            ...
                            return EndResponse.send(response, error.message, 500);
                        }
